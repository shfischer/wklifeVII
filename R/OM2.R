...
### load list with OMs
OM_list <- readRDS("input/stock_list.rds")
...
. <- foreach(x = seq_along(OM_list)) %do% {
  
  stk <- OM_list[[x]]$stk
  observations <- OM_list[[x]]$observations
  sr.om <- OM_list[[x]]$sr.om
  sr.om.res <- OM_list[[x]]$sr.om.res
  
  save(stk, observations, sr.om, sr.om.res, it, fy, y0, dy, iy, ny, nsqy, vy,
       file = paste0("input/stocks/perfect_knowledge/", 
                     OM_names[x],
                     "_new.RData"))
